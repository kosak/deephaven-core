...
public abstract class EndpointConfigBase(EndpointId id) {
  ...
  public static EndpointConfigBase OfCore(EndpointId id, string connectionString, bool sessionTypeIsPython) {
    return new CoreEndpointConfig(id, connectionString, sessionTypeIsPython);
  }
  ...
  public abstract T AcceptVisitor<T>(Func<CoreEndpointConfig, T> ofCore,
...
public sealed class CoreEndpointConfig(
  ...
  public override T AcceptVisitor<T>(Func<CoreEndpointConfig, T> ofCore, Func<CorePlusEndpointConfig, T> ofCorePlus) {
    return ofCore(this);
  }
}

public sealed class CorePlusEndpointConfig(EndpointId id, string jsonUrl, string user, string password,
  string operateAs, bool validateCertificate) : EndpointConfigBase(id) {
