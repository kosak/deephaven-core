...
public class TableHandle : IDisposable {
  public static TableHandle Create(TableHandleManager manager,
    ...
    var schema = server.FlightClient.GetSchema(fd, metadata).ResponseAsync.ConfigureAwait(false).GetAwaiter()
      .GetResult();
  ...
  public Table ToArrowTable() {
    ...
    while (reader.MoveNext().ConfigureAwait(false).GetAwaiter().GetResult()) {
      recordBatches.Add(reader.Current);
    }

    var schema = reader.Schema.ConfigureAwait(false).GetAwaiter().GetResult();
