...
public abstract class HMLFamacK1V1Base extends HMLFamacBase {
    HMLFamacK1V1Base(int desiredInitialCapacity, float loadFactor, long noEntryValue) {
        super(desiredInitialCapacity, loadFactor, noEntryValue);
    }

    static long reciprocalFor(long[] kvs) {
        return reciprocalFor(kvs.length / (1 * 2));
    }
    ...
    private static int getLocationFor(long[] kvs, long target, long numBucketsReciprocal) {
        ...
        final int probeStart = probe1(target, numBuckets, numBucketsReciprocal) * (1 * 2);
        ...
        while (true) {
            ...
            if (offset == 0) {
                offset = (1 + probe2(target, numBuckets - 2)) * (1 * 2);
            }
     ...
     * turn ends by loading the first key of its next bucket into the stash; the values are
     * consumed one full window-rotation later, by which time the cache line has typically arrived — so up to a
    ...
    final void getBatchImpl(long[] kvs, long numBucketsReciprocal, long[] keys, long[] result) {
        ...
        for (int w = 0; w < window; ++w) {
            ...
            final int probe = probe1(target, numBuckets, numBucketsReciprocal) * (1 * 2);
            jobSlot[w] = next;
            jobKey[w] = target;
            jobProbe[w] = probe;
        ...
        while (active > 0) {
            ...
            final int probe = jobProbe[w];
                ...
                if (k0 == target) {
                    value = kvs[probe + 1];
                } else if (k0 == SPECIAL_KEY_FOR_EMPTY_SLOT) {
                    value = noEntry;
                } else {
            ...
            if (next < n) {
                ...
                final int newProbe = probe1(newTarget, numBuckets, numBucketsReciprocal) * (1 * 2);
                jobSlot[w] = next;
                jobKey[w] = newTarget;
                jobProbe[w] = newProbe;
