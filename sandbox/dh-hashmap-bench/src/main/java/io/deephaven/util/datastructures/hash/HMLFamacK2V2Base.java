...
public abstract class HMLFamacK2V2Base extends HMLFamacBase {
    HMLFamacK2V2Base(int desiredInitialCapacity, float loadFactor, long noEntryValue) {
        super(desiredInitialCapacity, loadFactor, noEntryValue);
    }

    static long reciprocalFor(long[] kvs) {
        return reciprocalFor(kvs.length / (2 * 2));
    }
    ...
    private static int getLocationFor(long[] kvs, long target, long numBucketsReciprocal) {
        ...
        final int probeStart = probe1(target, numBuckets, numBucketsReciprocal) * (2 * 2);
        ...
        while (true) {
            ...
            if (offset == 0) {
                offset = (1 + probe2(target, numBuckets - 2)) * (2 * 2);
            }
    ...
    final void getBatchImpl(long[] kvs, long numBucketsReciprocal, long[] keys, long[] result) {
        ...
        for (int w = 0; w < window; ++w) {
            ...
            final int probe = probe1(target, numBuckets, numBucketsReciprocal) * (2 * 2);
            jobSlot[w] = next;
            jobKey[w] = target;
            jobProbe[w] = probe;
        ...
        while (active > 0) {
            ...
            final int probe = jobProbe[w];
                ...
                final long k1 = stashedKeyLast[w];
                final long value;
                if (k0 == target) {
                    value = kvs[probe + 1];
                } else if (k1 == target) {
                ...
                stashedKeyLast[w] = kvs[nextProbe + 2];
                continue;
            }
            result[slot] = value;
            if (next < n) {
                final long newTarget = fixKey(keys[next]);
                final int newProbe = probe1(newTarget, numBuckets, numBucketsReciprocal) * (2 * 2);
                jobSlot[w] = next;
                jobKey[w] = newTarget;
                jobProbe[w] = newProbe;
                ...
                stashedKeyLast[w] = kvs[newProbe + 2];
                ++next;
            } else {
                jobSlot[w] = -1;
                --active;
